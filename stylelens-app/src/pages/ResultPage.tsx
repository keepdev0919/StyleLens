...
interface AnalysisResult {
    ...
    bodyAnalysis: {
        upperBodyRatio: { label: string; score: number };
        stanceSymmetry: { label: string; score: number };
        colorAdaptability: { label: string; score: number };
        comment: string;
    };
}
...
export default function ResultPage() {
    ...
    const { palette, outfits, grooming, bodyAnalysis } = analysisResult;

    // Use user-uploaded photo if available, otherwise fallback
    // Note: In a real app, you'd upload the image to storage and get a public URL.
    // Here we use the local preview URL passed from InputPage, but passed state might need handling.
                ...
                <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-16">
                    <div className="max-w-2xl">
                        ...
                        <p className="text-slate-500 text-lg leading-relaxed max-w-xl">
                            A personalized breakdown of your unique aesthetic profile.
                            <br />
                            <span className="text-primary font-semibold">{palette.season}</span> palette detected.
                        </p>
                    </div>
                            ...
                            <div className="flex flex-col md:flex-row gap-12">
                                <div className="md:w-1/3">
                                    ...
                                    <p className="text-slate-500 text-sm leading-relaxed mb-8">
                                        Your seasonal type is <span className="text-primary font-bold">{palette.season}</span>.
                                        {palette.description}
                                    </p>
                                    <div className="flex flex-col gap-4">
                                        {palette.colors.slice(0, 3).map((color, idx) => (
                                            <div key={idx} className="flex items-center gap-3">
                                                ...
                                                <span className="text-sm font-semibold text-slate-700">{color.name}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                <div className="md:w-2/3 grid grid-cols-2 sm:grid-cols-4 gap-6">
                                    {palette.colors.map((item, index) => (
                                        <div key={index} className="flex flex-col gap-3">
                                            <div className="h-28 w-full rounded-2xl shadow-sm ring-1 ring-inset ring-black/5" style={{ backgroundColor: item.hex }}></div>
                                            <span className="text-[11px] font-mono text-center text-slate-400">{item.hex}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <section className="section-card rounded-2xl overflow-hidden">
                            <div className="p-8">
                                ...
                                <div className="bg-accent-pink rounded-2xl p-6 mb-8 border border-border-pink text-center">
                                    ...
                                    <div className="text-2xl font-bold text-slate-900">{grooming.faceShape}</div>
                                </div>
                                <div className="space-y-8">
                                    {grooming.tips.map((tip, idx) => (
                                        <div key={idx} className="flex items-start gap-5 group">
                                            {/* Placeholder for grooming images - reusing mock for now or logic */}
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500 mt-2 leading-relaxed">{tip.description}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="bg-primary/5 p-5 border-t border-border-pink">
                                <p className="text-[11px] font-bold text-center text-primary uppercase tracking-[0.15em]">Refinement Level: {grooming.refinementLevel}</p>
                            </div>
                        </section>
                        ...
                        <section className="section-card rounded-2xl p-8">
                            ...
                            <div className="space-y-8">
                                <div>
                                    <div className="flex justify-between text-[11px] font-bold mb-3 text-slate-400 tracking-wider">
                                        <span>UPPER BODY RATIO ({bodyAnalysis.upperBodyRatio.label})</span>
                                        <span className="text-primary">{bodyAnalysis.upperBodyRatio.score}%</span>
                                    </div>
                                    <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                        <div className="h-full bg-primary rounded-full" style={{ width: `${bodyAnalysis.upperBodyRatio.score}%` }}></div>
                                    </div>
                                </div>
                                <div>
                                    <div className="flex justify-between text-[11px] font-bold mb-3 text-slate-400 tracking-wider">
                                        <span>STANCE SYMMETRY ({bodyAnalysis.stanceSymmetry.label})</span>
                                        <span className="text-primary">{bodyAnalysis.stanceSymmetry.score}%</span>
                                    </div>
                                    <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                        <div className="h-full bg-primary rounded-full" style={{ width: `${bodyAnalysis.stanceSymmetry.score}%` }}></div>
                                    </div>
                                </div>
                                <div>
                                    <div className="flex justify-between text-[11px] font-bold mb-3 text-slate-400 tracking-wider">
                                        <span>COLOR ADAPTABILITY ({bodyAnalysis.colorAdaptability.label})</span>
                                        <span className="text-primary">{bodyAnalysis.colorAdaptability.score}%</span>
                                    </div>
                                    <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                        <div className="h-full bg-primary rounded-full" style={{ width: `${bodyAnalysis.colorAdaptability.score}%` }}></div>
                                    </div>
                                </div>
                            </div>
                            <div className="mt-10 pt-8 border-t border-pink-50">
                                <div className="flex items-start gap-4 p-5 rounded-2xl bg-accent-pink border border-border-pink">
                                    ...
                                    <p className="text-xs leading-relaxed text-slate-600 font-medium italic">"{bodyAnalysis.comment}"</p>
                                </div>
                            </div>
                        </section>
